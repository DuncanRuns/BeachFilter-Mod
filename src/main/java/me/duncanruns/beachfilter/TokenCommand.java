...
public class TokenCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(CommandManager.literal("beachtoken").executes(TokenCommand::execute));
    }

    private static int execute(CommandContext<ServerCommandSource> context) {
        String lastToken = BeachFilterMod.getLastToken();
        if (lastToken == null) {
        ...
        context.getSource().sendFeedback(
                new LiteralText("Last token: ").append(
                        Texts.bracketed(new LiteralText(lastToken).styled(style -> style.withColor(Formatting.GREEN)
                                .withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, BeachFilterMod.getLastToken()))
